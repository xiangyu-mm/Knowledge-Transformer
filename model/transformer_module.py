...
class TransformerBlock(nn.Module):
    def __init__(self, n_features, n_heads, dropout, attn_dropout, ff_dropout):
        ...
        self.layer_fusion = nn.Linear(100, n_features)

        self.attn_ent = MultiheadAttention(100, 4, attn_dropout)
        self.ent_norm = nn.LayerNorm(100)

    def forward(self, x, padding_mask, *contexts, x_ent=None, ent_mask=None):
        ...
        if x_ent is not None:
            full_attn_ent = self.attn_ent(x_ent,x_ent,x_ent,ent_mask.byte())
            x_ent = self.ent_norm(x_ent + full_attn_ent)
            #print(x_ent,x_ent.size())
            x_ent = x_ent * ent_mask.eq(0).unsqueeze(-1).float()
            x_ent = self.layer_fusion(x_ent)
            x = x + x_ent
        f = self.ff(x)
        f = self.dropout(f)
        x = self.ff_norm(x + f)
...
class TransformerModule(nn.Module):
    def __init__(self, n_layers, n_embeddings, n_pos_embeddings, embeddings_size, 
        ...
        '''
        layer_type = ["sim", "sim", "sim", "sim", "sim", "mix", "norm", "norm", "norm", "norm", "norm", "norm"]
        layers = []
        for t in layer_type:
            if t == "sim":
    ...
    def forward(self, x, enc_contexts=[], input_ent=None, ent_mask=None):
        
        padding_mask = x.eq(self.embeddings.padding_idx)
        if ent_mask is not None:
            ent_mask = ent_mask.eq(0)
        ...
        if self.n_segments is not None:
            ...
            for layer in self.layers:
                out = layer(x, padding_mask, *enc_contexts, x_ent=input_ent, ent_mask=ent_mask)
                x = out[0]
