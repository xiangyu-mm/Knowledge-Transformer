...
class FacebookDataset(Dataset):
    ...
    @staticmethod
    def make_dataset(data, max_lengths):
        dataset = []
        with open('truth_seq.txt','a+') as f:
            for chat in data:
                
                for s in chat['persona_info']:
                    f.write(s+' </s> ')
                  
                if len(chat['dialog']) % 2 == 1:
                    chat['dialog'] = chat['dialog'][:-1]
                
                #chat['dialog'] = chat['dialog'][-1]

                for s in chat['dialog']:
                    f.write(s+' </s> ')
                f.write('\n')
        f.close()
    ...
    def __init__(self, paths, max_lengths=2048, min_infos=2):
        assert min_infos > 0             
        ...
        self.data = FacebookDataset.make_dataset(parsed_data, max_lengths)
