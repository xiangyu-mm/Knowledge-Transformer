...
class Trainer:
    def __init__(self, model, train_dataset, test_dataset=None, vocab=None, batch_size=8,
        ...
        with open("kg_embed/entity2vec.vec", 'r') as fin:
            for line in fin:
                ...
                vecs.append(vec)
        self.embed = torch.FloatTensor(vecs)
        self.embed = torch.nn.Embedding.from_pretrained(self.embed)
    ...
    def load_state_dict(self, state_dict):
        ...
        #self.optimizer.load_state_dict(opt_dict)  #优化器重新训练

    def collate_func(self, data):
        persona_info, h, y, persona_info_ent, p_mask, h_ent, h_mask = zip(*data)
        ...
        if max(map(len, persona_info)) > 0:
            ...
            persona_info_ent = [torch.tensor(f, dtype=torch.long) for f in persona_info_ent]
            persona_info_ent = pad_sequence(persona_info_ent, batch_first=True, padding_value=-1)
            p_mask = [torch.tensor(f, dtype=torch.long) for f in p_mask]
            p_mask = pad_sequence(p_mask, batch_first=True, padding_value=0)
            contexts.append(persona_info)
            contexts_ent.append(persona_info_ent)
            ent_mask.append(p_mask)

        if max(map(len, h)) > 0:
            h = [torch.tensor(d, dtype=torch.long) for d in h]
            h = pad_sequence(h, batch_first=True, padding_value=self.model.padding_idx)
            h_ent = [torch.tensor(f, dtype=torch.long) for f in h_ent]
            h_ent = pad_sequence(h_ent, batch_first=True, padding_value=-1)
            h_mask = [torch.tensor(f, dtype=torch.long) for f in h_mask]
            h_mask = pad_sequence(h_mask, batch_first=True, padding_value=0)
            contexts.append(h)
            contexts_ent.append(h_ent)
            ent_mask.append(h_mask)
        y = [torch.tensor(d, dtype=torch.long) for d in y]
        y = pad_sequence(y, batch_first=True, padding_value=self.model.padding_idx)

        return contexts, y, contexts_ent, ent_mask
    ...
    def _eval_train(self, epoch, risk_func=None):
        ...
        lm_loss = 0
        risk_loss = 0
        for i, (contexts, targets, contexts_ent, ent_mask) in enumerate(tqdm_data):
            contexts, targets = [c.to(self.device) for c in contexts], targets.to(self.device) 
            contexts_ent = [self.embed(f+1).to(self.device) for f in contexts_ent]
            ent_mask = [t.to(self.device) for t in ent_mask]
            enc_contexts = []

            # lm loss
            batch_lm_loss = torch.tensor(0, dtype=torch.float, device=self.device)
            for j,context in enumerate(contexts):
                enc_context = self.model.encode(context.clone(), input_ent=contexts_ent[j].clone(), ent_mask = ent_mask[j].clone())
    ...
    def _eval_test(self, metric_funcs={}):
        ...
        for i, (contexts, targets, contexts_ent, ent_mask) in enumerate(tqdm_data):
            ...
            context_lens = contexts[0].ne(self.model.padding_idx).sum(dim=-1)
            contexts = [t[1:l-1].tolist() for t, l in zip(contexts[0], context_lens)]
            with open("log.txt", 'a') as fin:
                for prediction in predictions:
                    prediction_str = self.vocab.ids2string(prediction)
                    fin.writelines(prediction_str)
                    fin.writelines('\n')
            with open("truth.txt", 'a+') as fin:
                for target in targets:
                    target_str = self.vocab.ids2string(target)
                    fin.writelines(target_str)
                    fin.writelines('\n')

            lm_loss = (i * lm_loss + batch_lm_loss.item()) / (i + 1)
            loss = (i * loss + batch_loss.item()) / (i + 1)
            for name, func in metric_funcs.items():
                score1 = func(predictions, targets)
                score2 = func(predictions, contexts)
                score = score1 + score2
